...
def get_dataloader(args):
    """
    Эта функция загружает набор данных и напрямую извлекает все блоки изображений обучающей выборки в память, поэтому скорость использования памяти высока, что может легко привести к переполнению памяти.
    """
...
def get_dataloaderV2(args):
    """
    Эта функция загружает все изображения набора данных в память и создает индекс места, где извлекаются обучающие выборки, поэтому она занимает меньше памяти.
      Результаты теста показывают, что по сравнению с исходным методом get_dataloader, описанным выше, эффективность обучения не снизится.
    """
